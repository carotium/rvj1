module booth_core #(parameter int XLEN = 32) (
  ...
  logic [XLEN*2-1:0] op_a_2XLEN_sign_ext;
  logic [XLEN*2-1:0] op_b_2XLEN_sign_ext;
  ...
  generate
    for (i = 0; i < 16; i++) begin
      ...
      assign op_a_2XLEN_sign_ext = (op_a_ext[XLEN]) ? {{(XLEN*2 - XLEN){1'b1}}, op_a_ext} : {{(XLEN*2 - XLEN){1'b0}}, op_a_ext};
      assign op_b_2XLEN_sign_ext = (op_b_ext[XLEN]) ? {{(XLEN*2 - XLEN){1'b1}}, op_b_ext} : {{(XLEN*2 - XLEN){1'b0}}, op_b_ext};
